...
在GCC编译过程中，-S选项指示编译器对源代码进行预处理、编译，并在生成汇编代码后停止。也就是说，当你使用gcc -S source.c命令时，GCC不会执行链接步骤来生成可执行文件，而是输出一个.s或.asm格式的汇编语言源文件，该文件包含了从源代码翻译过来的汇编指令。

总结来说：

gcc -S: 生成汇编代码
输出文件：与源文件同名，但扩展名为 .s 或 .asm
不执行编译后的链接阶段
